...
# ======================================================= #
# ======================================================= #
qc_visual_rawcount = function(gene_sum, annorow, run){
  fileName =  paste0(gsub(".gene_summary.txt", "", basename(gene_sum)), "_", tolower(run))
  
  rawcount = read.table(file.path(dirname(dirname(gene_sum)), "rawcount", "rawcount.txt"), sep = "\t", header = T)
  
  Control = unlist(strsplit(as.character(annorow$Control), split = "\\,|\\;"))
  Sample =  unlist(strsplit(as.character(annorow$Sample), split = "\\,|\\;"))
  
  df = as.data.frame(t(rawcount[, c(Sample, Control)]))
  pca_res <- prcomp(df)
  df$condition = c(rep("Treatment", length(Sample)), rep("Control", length(Control)))
  df$batch = as.factor(c(1:length(Sample), 1:length(Control)))

  p1 = autoplot(pca_res, data = df, colour = 'condition', shape = "batch", size = 5, main = paste0(fileName, "\nrawCount_PCA"))
  ...
  png(filename = file.path((gsub(paste0("/", tolower(run), "/.*"), "", gene_sum)), 'qc', paste0(fileName, "_rawCount_corrPlot.png")), width = 6, height = 4, units = 'in', res = 300)
  draw(Heatmap(cor_res, col = colorRamp2(c(min(cor_res, na.rm = T), 1), c("white", "red")), column_title = paste0(fileName), name = "Pearson\nCorr"))
  dev.off()
}
# ======================================================= #
# ======================================================= #
qc_visual = function(gdata, gene_sum, topnames, run){
  # ===== prepare rank data 
  rankdata = gdata$Score
  names(rankdata) = gdata$id
  # ===== 
  fileName =  paste0(gsub(".gene_summary.txt", "", basename(gene_sum)), "_", tolower(run))
  fileTitle = paste0(gsub(".gene_summary.txt", "", basename(gene_sum)), "_", tolower(run))
  
  # ===================== #
  VolcanoView(gdata, x = "Score", y = "FDR", Label = "id", main = fileTitle,
              top = 0, topnames = topnames,
              filename = file.path((gsub(paste0("/", tolower(run), "/.*"), "", gene_sum)), 'qc', paste0(fileName, "_volcanoPlot.png")),
              width = 3, height = 1.8)
  # ===================== #
  RankView(rankdata = rankdata, main = fileTitle,
           top = 0, bottom = 0, genelist = topnames,
           filename = file.path((gsub(paste0("/", tolower(run), "/.*"), "", gene_sum)), 'qc', paste0(fileName, "_rankPlot.png")),
           width = 3, height = 1.8)
  # ===== 
  set.seed(200)
  gdata$RandomIndex = sample(dim(gdata)[1])
  
  p1 = ScatterView(data = gdata[gdata$Score<=0, ], x = "RandomIndex", y = "Score",
                   label = 'id', y_cut = -CutoffCalling(gdata$Score, 2),
                   groups = "bottom", group_col = 'blue',
                   toplabels = topnames, ylab = "Log2FC",
                   main = fileTitle, display_cut = TRUE)
  ggsave(plot = p1, filename = file.path((gsub(paste0("/", tolower(run), "/.*"), "", gene_sum)), 'qc', paste0(fileName, "_scatterBottom.png")),
         width = 3, height = 1.8, units = 'in', dpi = 300)
  
  p2 = ScatterView(data = gdata[gdata$Score>=0, ], x = "RandomIndex", y = "Score",
                   label = 'id', y_cut = CutoffCalling(gdata$Score, 2),
                   groups = "top", group_col = 'red',
                   toplabels = topnames, ylab = "Log2FC",
                   main = fileTitle, display_cut = TRUE)
  ggsave(plot = p2, filename = file.path((gsub(paste0("/", tolower(run), "/.*"), "", gene_sum)), 'qc', paste0(fileName, "_scatterTop.png")),
         width = 3, height = 1.8, units = 'in', dpi = 300)
  # ===== 
  gdata$sign = rep("0", dim(gdata)[1])
  gdata$sign[gdata$Score<0] = "neg"
  gdata$sign[gdata$Score>0] = "pos"
  gdata$sign = as.factor(gdata$sign)
  p3 = ggplot(gdata, aes(x=Score, color = sign)) +
    geom_histogram(fill="white",binwidth=0.2, alpha=0.5, position="identity") +
    labs(title = fileTitle, x = "Log2FC", y = "Count") +
    theme(text = element_text(size=6))
  ggsave(plot = p3, filename = file.path(gsub(paste0("/", tolower(run), "/.*"), "", gene_sum), 'qc', paste0(fileName, "_histogram.png")),
         width = 3, height = 1.8, units = 'in', dpi = 300)
}
# ======================================================= #
# ======================================================= #
...
step3_qc = function(gene_sum){
  ...
  # =====
  qc_visual_rawcount(gene_sum, annorow, run)
  qc_visual(gdata, gene_sum, topnames, run)
